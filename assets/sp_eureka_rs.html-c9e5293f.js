import{_ as e,p as n,q as t,a1 as a}from"./framework-d81ad7e5.js";const r={},i=a(`<p>eureka源码总结：</p><ol><li>client</li></ol><ul><li>拉取server注册表到本地。</li><li>注册服务。</li><li>初始化3个定时任务：心跳续约，定时拉取注册表，按需注册（InstanceInfoReplicator#run）。</li><li>发送下线。</li></ul><ol start="2"><li>server</li></ol><ul><li>接受服务注册</li><li>接受服务心跳</li><li>服务剔除</li><li>服务下线</li><li>集群同步（1.启动时从peer拉取信息，2.将注册到自己的服务同步到peer）</li></ul><h3 id="源码解读" tabindex="-1"><a class="header-anchor" href="#源码解读" aria-hidden="true">#</a> 源码解读</h3><p>下面的讲解按照顺序进行。</p><ul><li><p>spring boot项目引入eureka-client依赖，并注入spring 容器。</p><p>在spring-boot项目中pom文件里面添加的依赖中的bean。是如何注册到spring-boot项目的spring容器中的呢？spring.factories文件是帮助spring-boot项目包以外的bean（即在pom文件中添加依赖中的bean）注册到spring-boot项目的spring容器的。</p><p>由于@ComponentScan注解只能扫描spring-boot项目包内的bean并注册到spring容器中，因此需要@EnableAutoConfiguration（在SpringBootApplication下），注解来注册项目包外的bean。而spring.factories文件，则是用来记录项目包外需要注册的bean类名。</p><p>点进去@SpringBootApplication注解，发现@EnableAutoConfiguration。点@EnableAutoConfiguration进去。</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code>@Import<span class="token punctuation">(</span>AutoConfigurationImportSelector.class<span class="token punctuation">)</span>
public @interface EnableAutoConfiguration <span class="token punctuation">{</span>
</code></pre></div><p>点AutoConfigurationImportSelector进去</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code>发现下面代码
	@Override
	public String<span class="token punctuation">[</span><span class="token punctuation">]</span> selectImports<span class="token punctuation">(</span>AnnotationMetadata annotationMetadata<span class="token punctuation">)</span> <span class="token punctuation">{</span>
		<span class="token keyword">if</span> <span class="token punctuation">(</span><span class="token operator">!</span>isEnabled<span class="token punctuation">(</span>annotationMetadata<span class="token punctuation">))</span> <span class="token punctuation">{</span>
			<span class="token builtin class-name">return</span> NO_IMPORTS<span class="token punctuation">;</span>
		<span class="token punctuation">}</span>
		AutoConfigurationMetadata autoConfigurationMetadata <span class="token operator">=</span> AutoConfigurationMetadataLoader
				.loadMetadata<span class="token punctuation">(</span>this.beanClassLoader<span class="token punctuation">)</span><span class="token punctuation">;</span>
		AutoConfigurationEntry autoConfigurationEntry <span class="token operator">=</span> getAutoConfigurationEntry<span class="token punctuation">(</span>autoConfigurationMetadata,
				annotationMetadata<span class="token punctuation">)</span><span class="token punctuation">;</span>
		<span class="token builtin class-name">return</span> StringUtils.toStringArray<span class="token punctuation">(</span>autoConfigurationEntry.getConfigurations<span class="token punctuation">(</span><span class="token punctuation">))</span><span class="token punctuation">;</span>
	<span class="token punctuation">}</span>
</code></pre></div><p>此方法时，向spring ioc容器注入bean。selectImports，返回bean全名。import将bean全名注入。而注入的bean都是些什么呢？</p><p>点：getAutoConfigurationEntry进去，有一句</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code>List<span class="token operator">&lt;</span>String<span class="token operator">&gt;</span> configurations <span class="token operator">=</span> getCandidateConfigurations<span class="token punctuation">(</span>annotationMetadata, attributes<span class="token punctuation">)</span><span class="token punctuation">;</span>
</code></pre></div><p>点getCandidateConfigurations进去：</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code>List<span class="token operator">&lt;</span>String<span class="token operator">&gt;</span> configurations <span class="token operator">=</span> SpringFactoriesLoader.loadFactoryNames<span class="token punctuation">(</span>getSpringFactoriesLoaderFactoryClass<span class="token punctuation">(</span><span class="token punctuation">)</span>,
				getBeanClassLoader<span class="token punctuation">(</span><span class="token punctuation">))</span><span class="token punctuation">;</span>
			
</code></pre></div><p>点SpringFactoriesLoader进去：</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code>public static final String FACTORIES_RESOURCE_LOCATION <span class="token operator">=</span> <span class="token string">&quot;META-INF/spring.factories&quot;</span><span class="token punctuation">;</span>	
</code></pre></div></li><li><p>找eureka client 配置相关类</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code>在api-listen-order<span class="token punctuation">(</span>其他eureka client项目均可<span class="token punctuation">)</span>项目中，找到
...
</code></pre></div></li><li><p>EurekaDiscoveryClientConfiguration介绍</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code>找到此类：org.springframework.cloud.netflix.eureka.EurekaDiscoveryClientConfiguration中的注解@ConditionalOnClass<span class="token punctuation">(</span>EurekaClientConfig.class<span class="token punctuation">)</span>，
</code></pre></div></li><li><p>EurekaClientConfig介绍</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code>点击进去查看EurekaClientConfig是个接口，查看其实现类EurekaClientConfigBean。此类里封装了Eureka Client和Eureka Server交互所需要的配置信息。看此类代码：

  public static final String PREFIX <span class="token operator">=</span> <span class="token string">&quot;eureka.client&quot;</span><span class="token punctuation">;</span>
     表示在配置文件中用eureka.client.属性名配置。
</code></pre></div></li><li><p>Eureka 实例相关配置</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code>从org.springframework.cloud.client.discovery.DiscoveryClient顶级接口入手，前面介绍过spring common。看其在Eureka中的实现类org.springframework.cloud.netflix.eureka.EurekaDiscoveryClient。有一个属性：
...
有一个属性：
 ...
 private InstanceInfo instanceInfo<span class="token punctuation">;</span>
private EurekaInstanceConfig config<span class="token punctuation">;</span>
 服务实例的信息类InstanceInfo和服务实例配置信息类EurekaInstanceConfig）。
</code></pre></div></li><li><p>InstanceInfo介绍</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code>打开InstanceInfo里面有instanceId等服务实例信息。
 InstanceInfo封装了将被发送到Eureka Server进行注册的服务实例元数据。它在Eureka Server列表中代表一个服务实例，其他服务可以通过instanceInfo了解到该服务的实例相关信息，包括地址等，从而发起请求。
</code></pre></div></li><li><p>EurekaInstanceConfig介绍</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code>EurekaInstanceConfig是个接口，找到它的实现类org.springframework.cloud.netflix.eureka.EurekaInstanceConfigBean。
 此类封装了EurekaClient自身服务实例的配置信息，主要用于构建InstanceInfo。看到此类有一段代码：@ConfigurationProperties<span class="token punctuation">(</span><span class="token string">&quot;eureka.instance&quot;</span><span class="token punctuation">)</span>，
 在配置文件中用eureka.instance.属性配置。EurekaInstanceConfigBean提供了默认值。
</code></pre></div></li><li><p>通过EurekaInstanceConfig构建instanceInfo</p><div class="language-text" data-ext="text"><pre class="language-text"><code>在ApplicationInfoManager中有一个方法
...
</code></pre></div></li><li><p>顶级接口DiscoveryClient介绍</p><div class="language-text" data-ext="text"><pre class="language-text"><code>介绍一下spring-cloud-commons-2.2.1.realease包下，org.springframework.cloud.client.discovery.DiscoveryClient接口。定义用来服务发现的客户端接口，是客户端进行服务发现的核心接口，是spring cloud用来进行服务发现的顶级接口，在common中可以看到其地位。在Netflix Eureka和Consul中都有具体的实现类。
...
</code></pre></div></li><li><p>Eureka 的实现</p><div class="language-text" data-ext="text"><pre class="language-text"><code>接下来我们找Eureka的实现类。org.springframework.cloud.netflix.eureka.EurekaDiscoveryClient。
查看方法。
public List&lt;ServiceInstance&gt; getInstances(String serviceId)，
组合了com.netflix.discovery.EurekaClient来实现。
</code></pre></div></li><li><p>EurekaClient的实现</p><div class="language-text" data-ext="text"><pre class="language-text"><code>EurekaClient有一个注解@ImplementedBy(DiscoveryClient.class)，此类的默认实现类：com.netflix.discovery.DiscoveryClient。提供了:
服务注册到server方法register().
续约boolean renew().
下线public synchronized void shutdown().
查询服务列表 功能。
想想前面的图中client的功能。提供了于Eureka Server交互的关键逻辑。
</code></pre></div></li><li><p>com.netflix.discovery.DiscoveryClient</p><div class="language-text" data-ext="text"><pre class="language-text"><code>com.netflix.discovery.DiscoveryClient实现了EurekaClient（继承了LookupService）
</code></pre></div></li><li><p>com.netflix.discovery.shared.LookupService</p><div class="language-text" data-ext="text"><pre class="language-text"><code>LookupService作用：发现活跃的服务实例。
...
</code></pre></div></li><li><p>健康检测器和事件监听器</p><div class="language-text" data-ext="text"><pre class="language-text"><code>EurekaClient在LookupService上做了扩充。提供了更丰富的获取服务实例的方法。按住不表。我们看一下另外两个方法：
...
</code></pre></div></li><li><p>找到com.netflix.discovery.DiscoveryClient</p><div class="language-text" data-ext="text"><pre class="language-text"><code>在api-listen-order项目中，找到spring-cloud-netflix-eureka-client-2.1.2.RELEASE下META-INF下spring.factories。此文件中org.springframework.cloud.bootstrap.BootstrapConfiguration=\\
...
</code></pre></div></li><li><p>com.netflix.discovery.DiscoveryClient构造函数-不注册不拉取</p><div class="language-text" data-ext="text"><pre class="language-text"><code>DiscoveryClient的构造函数：
...
</code></pre></div></li><li><p>com.netflix.discovery.DiscoveryClient构造函数-两个定时任务</p><div class="language-text" data-ext="text"><pre class="language-text"><code>顺着上面代码往下看：
...
</code></pre></div></li><li><p>com.netflix.discovery.DiscoveryClient构造函数-client和server交互的Jersey客户端</p><div class="language-text" data-ext="text"><pre class="language-text"><code>接着构建eurekaTransport = new EurekaTransport();它是eureka Client和eureka server进行http交互jersey客户端。点开EurekaTransport，看到许多httpclient相关的属性。

</code></pre></div></li><li><p>com.netflix.discovery.DiscoveryClient构造函数-拉取注册信息</p><div class="language-text" data-ext="text"><pre class="language-text"><code>  if (clientConfig.shouldFetchRegistry() &amp;&amp; !fetchRegistry(false)) {
              fetchRegistryFromBackup();
  }
  如果判断的前部分为true，执行后半部分fetchRegistry。此时会从eureka server拉取注册表中的信息，将注册表缓存到本地，可以就近获取其他服务信息，减少于server的交互。
</code></pre></div></li><li><p>com.netflix.discovery.DiscoveryClient构造函数-服务注册</p><div class="language-text" data-ext="text"><pre class="language-text"><code> if (clientConfig.shouldRegisterWithEureka() &amp;&amp; clientConfig.shouldEnforceRegistrationAtInit()) {
              try {
                  if (!register() ) {
...
</code></pre></div></li><li><p>com.netflix.discovery.DiscoveryClient构造函数-启动定时任务</p><div class="language-text" data-ext="text"><pre class="language-text"><code>在构造方法的最后initScheduledTasks();此方法中，启动3个定时任务。方法内有statusChangeListener，按需注册是一个事件StatusChangeEvent，状态改变，则向server注册。
</code></pre></div></li><li><p>com.netflix.discovery.DiscoveryClient构造函数-总结</p><div class="language-text" data-ext="text"><pre class="language-text"><code>总结DiscoveryClient构造关键过程：
  初始化一堆信息。
  从拉取注册表信息。
  向server注册自己。
  初始化3个任务。
  详细后面继续讲。源码就是这样，得层层拨开。
</code></pre></div></li><li><p>拉取注册表信息详解</p><div class="language-text" data-ext="text"><pre class="language-text"><code>上面的fetchRegistry(false)，点进去，看注释：
...
</code></pre></div></li><li><p>全量拉取</p><div class="language-text" data-ext="text"><pre class="language-text"><code>进入getAndStoreFullRegistry() 方法，有一方法：eurekaTransport.queryClient.getApplications。
...
</code></pre></div></li><li><p>增量拉取</p><div class="language-text" data-ext="text"><pre class="language-text"><code>回到刚才的fetchRegistry方法中，getAndUpdateDelta，增量拉取。通过getDelta方法，看到实际拉取的地址是：apps/delta，如果获取到的delta为空，则全量拉取。
...
</code></pre></div></li><li><p>服务注册</p><div class="language-text" data-ext="text"><pre class="language-text"><code>好了拉取完eureka server中的注册表了，接着进行服务注册。回到DiscoveryClient构造函数。
...
</code></pre></div></li><li><p>初始化3个定时任务</p><div class="language-text" data-ext="text"><pre class="language-text"><code>接着
...
</code></pre></div></li><li><p>服务下线</p><div class="language-text" data-ext="text"><pre class="language-text"><code>服务下线：在应用关闭时，client会向server注销自己，在Discoveryclient销毁前，会执行下面清理方法。 
@PreDestroy
@Override
...
</code></pre></div><ul><li><p>client源码总结</p><div class="language-text" data-ext="text"><pre class="language-text"><code>总结：源码其实两部分内容：
1、client自身的操作。
2、server的配合。（https://github.com/Netflix/eureka/wiki/Eureka-REST-operations）。
  一切尽在：《Eureka Client工作流程图》
</code></pre></div></li></ul></li></ul><h2 id="_11-8-eureka-server源码" tabindex="-1"><a class="header-anchor" href="#_11-8-eureka-server源码" aria-hidden="true">#</a> 11.8 Eureka Server源码</h2><ol><li><p>Eureka Server功能复习</p><p>接受服务注册 接受服务心跳 服务剔除 服务下线 集群同步 获取注册表中服务实例信息</p><p>需要注意的是，Eureka Server同时也是一个Eureka Client，在不禁止Eureka Server的客户端行为时，它会向它配置文件中的其他Eureka Server进行拉取注册表、服务注册和发送心跳等操作。</p></li><li><p>源码解读</p><ul><li><p>启动server注册相关bean</p><div class="language-text" data-ext="text"><pre class="language-text"><code>注册外部的配置类
...
</code></pre></div></li><li><p>开启eureka server</p><div class="language-text" data-ext="text"><pre class="language-text"><code> 开关：
...
</code></pre></div></li><li><p>开启注册</p><div class="language-text" data-ext="text"><pre class="language-text"><code>在EurekaServerMarkerConfiguration上有@Import(EurekaServerInitializerConfiguration.class)，导入了EurekaServerInitializerConfiguration，
EurekaServerInitializerConfiguration
		implements ServletContextAware, SmartLifecycle，SmartLifecycle的作用是：初始化完之后，
执行public void start()方法。
		
</code></pre></div></li></ul></li></ol><p>在public void start()中，启动一个线程。看注释：log.info(&quot;Started Eureka Server&quot;);发布事件：publish(new EurekaRegistryAvailableEvent(getEurekaServerConfig()))， 告诉client，可以来注册了。</p><div class="language-text" data-ext="text"><pre class="language-text"><code> 上面提到的 log.info(&quot;Started Eureka Server&quot;) 的上面一行。eurekaServerBootstrap.contextInitialized(EurekaServerInitializerConfiguration.this.servletContext);
...
</code></pre></div><ul><li><p>接受服务注册</p><blockquote><p>《eureka服务端注册》</p></blockquote><div class="language-text" data-ext="text"><pre class="language-text"><code>我们学过Eureka Client在发起服务注册时会将自身的服务实例元数据封装在InstanceInfo中，然后将InstanceInfo发送到Eureka Server。Eureka Server在接收到Eureka Client发送的InstanceInfo后将会尝试将其放到本地注册表中以供其他Eureka Client进行服务发现。
...
-----------
</code></pre></div></li></ul><div class="language-text" data-ext="text"><pre class="language-text"><code>public javax.ws.rs.core.Application jerseyApplication(方法：中。
 provider.addIncludeFilter(new AnnotationTypeFilter(Path.class));
		provider.addIncludeFilter(new AnnotationTypeFilter(Provider.class));
 添加一些过滤器，类似于过滤请求地址，Path类似于@RequestMapping，Provider类似于@Controller		
 
</code></pre></div><hr><div class="language-text" data-ext="text"><pre class="language-text"><code> 在com.netflix.eureka.resources包下，是Eureka Server对于Eureka client的REST请求的定义。看ApplicationResource类（这是一类请求，应用类的请求），类似于应用@Controller注解：@Produces({&quot;application/xml&quot;, &quot;application/json&quot;})，接受xml和json。见名识意 public Response addInstance。添加实例instanceinfo。 方法中，有一句：
...
</code></pre></div><p>​</p><ul><li><p>接受心跳 续租，renew</p><blockquote><p>《Eureka服务端接收心跳》</p></blockquote><div class="language-text" data-ext="text"><pre class="language-text"><code>在Eureka Client完成服务注册之后，它需要定时向Eureka Server发送心跳请求(默认30秒一次)，维持自己在Eureka Server中租约的有效性。
...
点击renew的实现。
进入下面正题：
</code></pre></div></li></ul><div class="language-text" data-ext="text"><pre class="language-text"><code>Eureka Server处理心跳请求的核心逻辑位于AbstractInstanceRegistry#renew方法中。renew方法是对Eureka Client位于注册表中的租约的续租操作，不像register方法需要服务实例信息，仅根据服务实例的服务名和服务实例id即可更新对应租约的有效时间。
 com.netflix.eureka.registry.AbstractInstanceRegistry
renew
...
</code></pre></div><ul><li><p>服务剔除</p><div class="language-text" data-ext="text"><pre class="language-text"><code>如果Eureka Client在注册后，既没有续约，也没有下线(服务崩溃或者网络异常等原因)，那么服务的状态就处于不可知的状态，不能保证能够从该服务实例中获取到回馈，所以需要服务剔除此方法定时清理这些不稳定的服务，该方法会批量将注册表中所有过期租约剔除。
...
1.自我保护期间不清除。
2.分批次清除。
</code></pre></div></li></ul><div class="language-text" data-ext="text"><pre class="language-text"><code>3.服务是逐个随机剔除，剔除均匀分布在所有应用中，防止在同一时间内同一服务集群中的服务全部过期被剔除，造成在大量剔除服务时，并在进行自我保护时，促使程序崩溃。

</code></pre></div><p>EurekaServerInitializerConfiguration的 eurekaServerBootstrap.contextInitialized(方法，中initEurekaServerContext();点进去this.registry.openForTraffic(this.applicationInfoManager, registryCount);点进去，super.postInit();点进去evictionTaskRef.set(new EvictionTask()); evictionTimer.schedule(evictionTaskRef.get(), serverConfig.getEvictionIntervalTimerInMs(), serverConfig.getEvictionIntervalTimerInMs()); 发现 定时任务。</p><div class="language-text" data-ext="text"><pre class="language-text"><code> 剔除服务是个定时任务，用EvictionTask执行，默认60秒执行一次，延时60秒执行。定时剔除过期服务。
...
</code></pre></div><p>​<br> ​ 自我保护机制主要在Eureka Client和Eureka Server之间存在网络分区的情况下发挥保护作用，在服务器端和客户端都有对应实现。假设在某种特定的情况下(如网络故障)，Eureka Client和Eureka Server无法进行通信，此时Eureka Client无法向Eureka Server发起注册和续约请求，Eureka Server中就可能因注册表中的服务实例租约出现大量过期而面临被剔除的危险，然而此时的Eureka Client可能是处于健康状态的(可接受服务访问)，如果直接将注册表中大量过期的服务实例租约剔除显然是不合理的。 ​ 针对这种情况，Eureka设计了“自我保护机制”。在Eureka Server处，如果出现大量的服务实例过期被剔除的现象，那么该Server节点将进入自我保护模式，保护注册表中的信息不再被剔除，在通信稳定后再退出该模式；在Eureka Client处，如果向Eureka Server注册失败，将快速超时并尝试与其他的Eureka Server进行通信。“自我保护机制”的设计大大提高了Eureka的可用性。 ​ \`\`\`</p><ul><li><p>服务下线</p><blockquote><p>《Eureka服务下线》</p></blockquote><div class="language-text" data-ext="text"><pre class="language-text"><code>Eureka Client在应用销毁时，会向Eureka Server发送服务下线请求，清除注册表中关于本应用的租约，避免无效的服务调用。在服务剔除的过程中，也是通过服务下线的逻辑完成对单个服务实例过期租约的清除工作。
...
</code></pre></div></li></ul><div class="language-text" data-ext="text"><pre class="language-text"><code>添加到最近下线服务的统计队列
...
</code></pre></div><ul><li><p>集群同步</p><div class="language-text" data-ext="text"><pre class="language-text"><code>如果Eureka Server是通过集群的方式进行部署，那么为了维护整个集群中Eureka Server注册表数据的一致性，势必需要一个机制同步Eureka Server集群中的注册表数据。
...
</code></pre></div></li></ul><div class="language-text" data-ext="text"><pre class="language-text"><code>在PeerAwareInstanceRegistryImpl类中，看其他操作，cancel，renew等中都有replicateToPeers，
