...
</code></pre></div><h4 id="_3-解决办法" tabindex="-1"><a class="header-anchor" href="#_3-解决办法" aria-hidden="true">#</a> 3. 解决办法</h4>`,17),m={href:"https://goaccess.io/download",target:"_blank",rel:"noopener noreferrer"},x=t(`<div class="language-bash" data-ext="sh"><pre class="language-bash"><code>$ yum <span class="token function">install</span> geoip-devel ncurses-devel
</code></pre></div><h4 id="_4-相关资料" tabindex="-1"><a class="header-anchor" href="#_4-相关资料" aria-hidden="true">#</a> 4. 相关资料</h4>`,2),b={href:"https://goaccess.io/",target:"_blank",rel:"noopener noreferrer"},f=t(`<h3 id="配置选项" tabindex="-1"><a class="header-anchor" href="#配置选项" aria-hidden="true">#</a> 配置选项</h3><div class="language-text" data-ext="text"><pre class="language-text"><code>GoAccess 拥有多个配置选项。获取完整的最新配置选项列表，请运行：./configure --help
...
</code></pre></div><div class="custom-container danger"><p class="custom-container-title">DANGER</p><p>goaccess -f /usr/local/nginx/logs/access.log -o /usr/local/nginx/html/report.html --log-format=COMBINED --real-time-html</p></div><p>启动服务后会提示开启了一个 websocket</p><p>接着就可以在浏览器访问对应的地址了。</p><div class="language-bash" data-ext="sh"><pre class="language-bash"><code><span class="token function">wget</span> https://github.com/maxmind/libmaxminddb/releases/download/1.3.2/libmaxminddb-1.3.2.tar.gz
<span class="token function">tar</span> <span class="token parameter variable">-xvf</span> libmaxminddb-1.3.2.tar.gz
<span class="token builtin class-name">cd</span> libmaxminddb-1.3.2
<span class="token comment"># 备用下载地址：https://down.24kplus.com/linux/libmaxminddb-1.3.2.tar.gz</span>
...
<span class="token function">npm</span> <span class="token function">install</span> <span class="token parameter variable">-y</span>   <span class="token operator">&gt;&gt;</span><span class="token operator">&gt;&gt;</span><span class="token operator">&gt;&gt;</span>  ncurses ncurses-devel geoip geoip-devel
</code></pre></div>`,14);function k(_,v){const n=c("ExternalLinkIcon");return l(),r("div",null,[d,e("p",null,[a("虽然 Nginx 已经提供 access 日志来查阅，但是不得不说很难看清，常规的正则过滤又显得较为麻烦。于是就找到了这么一款实时网络日志分析器 "),e("a",p,[a("GoAccess"),s(n)]),a("。")]),h,e("ul",null,[e("li",null,[a("缺少的包可以去"),e("a",g,[a("aliyun"),s(n)]),a("下载")])]),u,e("p",null,[a("官方已经给出了各个平台对应依赖包的名字（"),e("a",m,[a("传送至官网"),s(n)]),a("），只需使用对应平台的包管理工具安装即可 如以上两个报错在 centos 下只需安装 geoip-devel 和 ncurses-devel 即可：")]),x,e("p",null,[e("a",b,[a("GoAccess 官网：https://goaccess.io"),s(n)])]),f])}const w=o(i,[["render",k],["__file","goaccess.html.vue"]]);export{w as default};
